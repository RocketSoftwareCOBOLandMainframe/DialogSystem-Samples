       ...
       01 _pCustomerDataBlock      type IntPtr.
       78  no-of-orders            VALUE 10.

      *> pData represents the CUSTOMER-DATA-BLOCK record packed into a byte array object
       01 pData type Byte occurs any. 
       copy "CUSTOMER.CPB".
       
       method-id NEW.
       procedure division using by value callback as type CustomerCallback.
       
           *> Call back to native code to retrieve customer data block. We stash the pointer in _pCustomerDataBlock
       ...
       procedure division using by value sender as object e as type System.EventArgs.
           ...
           *> Unpack the data block to our own copy in local storage. 
           *> We can then access it just as we would in native code
           set size of pData to length of CUSTOMER-DATA-BLOCK
           invoke type Marshal::Copy(_pCustomerDataBlock, pData, 0, LENGTH OF CUSTOMER-DATA-BLOCK)
      ...
      *>> <param name="e"></param>
       ...
       procedure division using by value sender as object e as type System.Windows.Forms.FormClosingEventArgs.
           ...
           *> Move the data block back to native code so that changes can be picked up 
           set pData to CUSTOMER-DATA-BLOCK       
           invoke type Marshal::Copy(pData, 0, _pCustomerDataBlock, LENGTH OF CUSTOMER-DATA-BLOCK)
           goback.
       end method.
